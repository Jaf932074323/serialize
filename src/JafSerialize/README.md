JafSerialize
====
* 将C++数据结构序列化成字节数组，以及将字节数组反序列化成C++数据结构
* 设计初衷是为了在处理网络通讯时，能够方便的转换结构体和字节数组，方便的处理大小端问题，方便的处理复杂多变的数据结构
* 本序列化库是在开源库（https://github.com/Jaf932074323/Serialization-and-Deserialization-Data.git）的基础上，借鉴开源库xpack（https://github.com/xyz347/xpack.git）的部分设计，改良得到
* 只有头文件, 没有cpp文件
* TestJafSerialize文件夹中包含了测试用例，也可以当作用法例子进行参考
